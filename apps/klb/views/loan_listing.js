    ...
    */
    render: function(context, firstTime) {
      var content = this.get('content'),
        nameLabel, picture, amountLabel, loanNumbers, useLabel, sectorLabel, 
        countryLabel, pictureHolder, partnerLabel;
      pictureHolder = context.begin('img')
        .attr('src', 'https://s3.amazonaws.com/s3.kiva.org/img/w80h80/'+content.get('image').id+'.jpg')
        .addClass('klb-llv-picture').end();
      nameLabel = context.begin('div').addClass('klb-llv-name')
        .push(content.get('name')).end();
      sectorLabel = context.begin('div').addClass('klb-llv-sector')
          .push(content.get('sector')).end();
      loanNumbers = context.begin('div').addClass('klb-llv-loanNumbers')
        .push('<span class="bigNumber">$').push(content.get('fundedAmount')+'</span><br/>')
        .push('<span>needed of $').push(content.get('loanAmount')+'</span>').end();
      useLabel = context.begin('div').addClass('klb-llv-loanUse')
        .push('Loan Use: ').push(content.get('use')).end();
      countryLabel = context.begin('div').addClass('klb-llv-country')
        .push(content.get('country').country_code).end();
      partnerLabel = context.begin('div').addClass('klb-llv-partner')
          .push(content.get('partner_id')).end();
